...
import net.codejava.com.mvc.bean.LoginBean;
import net.codejava.com.mvc.dao.LoginDao;
...
else if(userValidate.equals("SUCCESS_CLIENT")) //If function returns success string then user will be rooted to Home page
{
request.setAttribute("userName", userName); //with setAttribute() you can define a "key" and value pair so that you can get it in future using getAttribute("key")
request.getRequestDispatcher("/ClientHome.jsp").forward(request, response);//RequestDispatcher is used to send the control to the invoked page.
}
else
{
